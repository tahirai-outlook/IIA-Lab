...
1. スタート メニューを開いて **PowerShell** と入力し、続いて **\[管理者として実行]** をクリックします。
...
1. 現在の JSON 文字列を以下のものと置き換えます。その際、**\[module name]** を前の手順でメモしたモジュール名に変更してください。**\[module name]** は、全部で 3 か所変更する必要があります。
